...
class findidcard:
    ...
    def find(self, img2_name):
        ...
        if len(good)>MIN_MATCH_COUNT:
            ...
            # 保存整张身份证图片
            path_name = './saved'
            head_name = '%s/%d.jpg' % (path_name, 1)
            cv2.imwrite(head_name, im_r)

            # 打开保存的图片，抠出头像重新保存
            im = cv2.imread(head_name, 1)
            crop = im[80:280, 410:590]
            # cv2.imshow("image",im)
            # cv2.waitKey(0)

            path_name1 = './head'
            head_name1 = '%s/%d.jpg' % (path_name1, 1)
            cv2.imwrite(head_name1, crop)
        else:
